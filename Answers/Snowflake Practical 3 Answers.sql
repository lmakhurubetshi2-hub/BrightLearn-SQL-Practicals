...
SELECT CUSTOMER_ID,
       ...
       purchase_amount,
       Item_Purchased
FROM practical3.shopping.trends
...
SELECT IFNULL(frequency_of_purchases, 'Unknown') AS frequency_of_purchases,
       SUM(purchase_amount) AS Total_Purchases
FROM practical3.shopping.trends
GROUP BY  IFNULL(frequency_of_purchases, 'Unknown');
...
SELECT
    location,
    SUM(IFNULL(previous_purchases,0)) AS Total_Purchases
FROM practical3.shopping.trends
GROUP BY location
ORDER BY SUM(IFNULL(previous_purchases,0)) DESC
LIMIT 5;
